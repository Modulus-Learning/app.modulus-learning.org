#!/usr/bin/env bash
#
# publish-packages.sh — publish @modulus-learning/agent to npm under passkey 2FA.
#
...
#
# This replaces step 3 (`pnpm release:npm`) of the manual release flow:
#   1. pnpm changeset           # describe the changeset
#   2. pnpm version-packages    # version the package(s) (commit the result)
#   3. ./publish-packages.sh    # build, pack, publish, tag, push
#
# It builds, packs, publishes, and pushes the release tag. It is idempotent: a
# package already live at the current version is skipped, and an existing tag is
# not recreated — so re-running after a partial failure just finishes the job.
#
# Usage:
...
PKG_NAME="@modulus-learning/agent"
PKG_DIR="apps/agent"
...
cleanup_tarballs() {
  find "$REPO_ROOT/${PKG_DIR}" -maxdepth 1 -name '*.tgz' -delete 2>/dev/null || true
}
...
# Warn if the working tree is dirty — the version bump from `pnpm version-packages`
# should already be committed before publishing/tagging.
if [ -n "$(git status --porcelain)" ]; then
  warn "Working tree is not clean:"
  git status --short
  warn "The tag will point at $ANCHOR_SHORT regardless. Commit the version bump (and stash/revert unrelated changes) before tagging."
  echo
fi
