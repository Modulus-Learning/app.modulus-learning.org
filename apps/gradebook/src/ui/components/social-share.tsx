'use client'

import { useEffect, useState } from 'react'

import {
  EmailIcon,
  EmailShareButton,
  FacebookIcon,
  FacebookShareButton,
  LinkedinIcon,
  LinkedinShareButton,
  TwitterShareButton,
  XIcon,
} from 'react-share'
import { twMerge } from 'tailwind-merge'

interface Share {
  title: string
  url: string
}

export function SocialShare({ className }: { className?: string }): React.JSX.Element | null {
  const [share, setShare] = useState<Share | null>(null)

  useEffect(() => {
    if (share == null) {
      setShare({ title: document.title, url: window.location.href })
    }
  }, [share])

  if (share == null) {
    return null
  }

  return (
    <div className={twMerge('flex flex-wrap items-center gap-2 justify-start', className)}>
      <div className="flex gap-1 items-center mr-1">
        <svg
          className="w-[18px] fill-black dark:fill-gray-300"
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          viewBox="0 0 30 30"
        >
          <title>Share</title>
          <path d="M 23 3 C 20.791 3 19 4.791 19 7 C 19 7.2869826 19.034351 7.5660754 19.091797 7.8359375 L 10 12.380859 C 9.2667379 11.541629 8.2018825 11 7 11 C 4.791 11 3 12.791 3 15 C 3 17.209 4.791 19 7 19 C 8.2018825 19 9.2667379 18.458371 10 17.619141 L 19.091797 22.164062 C 19.034351 22.433925 19 22.713017 19 23 C 19 25.209 20.791 27 23 27 C 25.209 27 27 25.209 27 23 C 27 20.791 25.209 19 23 19 C 21.798117 19 20.733262 19.541629 20 20.380859 L 10.908203 15.835938 C 10.965649 15.566075 11 15.286983 11 15 C 11 14.713017 10.965649 14.433925 10.908203 14.164062 L 20 9.6191406 C 20.733262 10.458371 21.798117 11 23 11 C 25.209 11 27 9.209 27 7 C 27 4.791 25.209 3 23 3 z" />
        </svg>
        <span className="inline-block text-[0.9rem] whitespace-nowrap">Share</span>
      </div>

      <FacebookShareButton
        url={share.url}
        aria-label="Share to Facebook"
        className="cursor-pointer transition-all duration-200 hover:opacity-75"
      >
        <FacebookIcon size={26} round />
      </FacebookShareButton>
      <TwitterShareButton
        url={share.url}
        aria-label="Share to Twitter / X"
        title={share.title}
        className="cursor-pointer transition-all duration-200 hover:opacity-75"
      >
        <XIcon size={26} round />
      </TwitterShareButton>
      <LinkedinShareButton
        url={share.url}
        aria-label="Share to LinkedIn"
        className="cursor-pointer transition-all duration-200 hover:opacity-75"
      >
        <LinkedinIcon size={26} round />
      </LinkedinShareButton>
      <EmailShareButton
        url={share.url}
        aria-label="Share via Email"
        subject={share.title}
        body="body"
        className="cursor-pointer transition-all duration-200 hover:opacity-75"
      >
        <EmailIcon size={26} round />
      </EmailShareButton>
    </div>
  )
}
