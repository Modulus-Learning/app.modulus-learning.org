...
import type { AdminUserFormState, AdminUserResponse } from '../@types'
...
export function AdminUserEditForm({ data, lng }: UserEditFormProps): React.JSX.Element {
  const { theme } = useTheme()
  const initialState: AdminUserFormState = { message: undefined, errors: {}, status: 'idle' }
  const [tab, setTab] = useState('detailsTab')
  const [formState, formAction, isPending] = useActionState(editAdminUser, initialState)
  const resolver = zodResolver(adminUserEditSchema)
  const {
    control,
    register,
  ...
  type ErrorKeys = keyof AdminUserFormState['errors']
  ...
  return (
    <div className="max-w-[500px] mx-auto rounded-md border border-gray-100 dark:border-gray-700 p-5 pb-1 mb-8 mt-[4vh]">
      <h2 className="!m-0 !mb-4">Edit Admin User</h2>
      ...
      >
        ...
        <div className="form-actions flex gap-2 justify-end my-4">
          <Button
            ...
            intent="noeffect"
            render={<LangLink href={`/admin/admin-users/${data?.user?.id}`} lng={lng} />}
          >
