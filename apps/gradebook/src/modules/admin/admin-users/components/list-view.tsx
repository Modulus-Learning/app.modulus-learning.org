...
export function AdminUsersListView({ data, lng }: { data: AdminUsersResponse; lng: Locale }) {
  ...
  const handleOnSearch = (query: string): void => {
    if (query != null && query.length > 0) {
      ...
      navigate({ href: `/admin/admin-users?${searchParams?.toString()}` as string, scroll: false })
    }
  }
  ...
  function handleOnPageSizeChange(value: unknown): void {
    ...
    if (v != null && v.length > 0) {
      ...
      navigate({
        href: `/admin/admin-users?${searchParams?.toString()}` as string,
        scroll: true,
      ...
      <Container>
        <div className="flex items-center gap-3 py-[2px]">
          ...
          <IconButton
            ...
            render={<LangLink href="/admin/admin-users/add" lng={lng} />}
          >
              ...
              {data?.users?.map((user) => {
                return (
                  <Table.Row key={user.id} className={user.is_enabled ? '' : 'text-red-500'}>
                    <Table.Cell>
                      <LangLink href={`/admin/admin-users/${user.id}`} lng={lng}>
                        {user.given_name}
                      </LangLink>
                    </Table.Cell>
