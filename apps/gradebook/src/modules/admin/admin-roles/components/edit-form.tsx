...
import { adminRoleEditSchema } from '../@types'
import { editAdminRole } from '../edit'
import type { Locale } from '@/i18n/i18n-config'
import type { AdminRoleFormState, AdminRoleResponse } from '../@types'

interface AdminRoleEditFormProps {
  lng: Locale
  data: AdminRoleResponse
}

export function AdminRoleEditForm({ data, lng }: AdminRoleEditFormProps): React.JSX.Element {
  const { theme } = useTheme()
  const initialState: AdminRoleFormState = { message: undefined, errors: {}, status: 'idle' }
  const [tab, setTab] = useState('detailsTab')
  const [formState, formAction, isPending] = useActionState(editAdminRole, initialState)
  const resolver = zodResolver(adminRoleEditSchema)
  const {
    // control,
    register,
  ...
  type ErrorKeys = keyof AdminRoleFormState['errors']
  ...
  return (
    <div className="max-w-[500px] mx-auto rounded-md border border-gray-100 dark:border-gray-700 p-5 pb-1 mb-8 mt-[4vh]">
      <h2 className="!m-0 !mb-4">Edit Admin Role</h2>
      ...
      >
        ...
        <div className="form-actions flex gap-2 justify-end my-4">
          <Button
            ...
            intent="noeffect"
            render={<LangLink href={`/admin/admin-roles/${data.role?.id}`} lng={lng} />}
          >
