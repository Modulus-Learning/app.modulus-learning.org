...
import { RoleContextMenu } from './context-menu'
import type { Locale } from '@/i18n/i18n-config'
import type { RoleDeleteState, RoleResponse } from '../@types'
...
export function RoleDetailsView({ lng, data, mode }: UserDetailsProps): React.JSX.Element {
  const { theme } = useTheme()
  const [tab, setTab] = useState('detailsTab')
  const initialState: RoleDeleteState = { message: undefined, status: 'idle' }
  const [formState, formAction, _isPending] = useActionState(deleteRole, initialState)
  ...
  return (
    <div className="max-w-[500px] mx-auto rounded-md border border-gray-100 dark:border-gray-700 px-5 pb-1 mb-8 mt-[4vh]">
      <div className="py-3 mb-2 flex items-center justify-start">
        <h2 className="!m-0 !mr-auto">{mode === 'delete' ? 'Delete  Role' : ' Role Details'}</h2>
        <RoleContextMenu lng={lng} data={data} mode={mode} />
      </div>
      ...
      <div className="form-actions flex gap-2 justify-end my-4">
        <Button
          ...
          render={
            <LangLink
              href={mode === 'delete' ? `/admin/roles/${data.role?.id}` : '/admin/roles'}
              lng={lng}
            />
          }
        >
