...
import { orderRoles } from '../order'
import type { Locale } from '@/i18n/i18n-config'
import type { Role, RoleOrderState, RolesResponse } from '../@types'
...
const DraggableRow: React.FC<DraggableRowProps> = ({ item, disabled, isSortable = true, lng }) => {
  ...
  return (
    ...
    >
      ...
      <Table.Cell>
        <LangLink href={`/admin/roles/${item.id}`} lng={lng}>
          {item.name}
        </LangLink>
      </Table.Cell>
...
export function RolesListView({ data, lng }: { data: RolesResponse; lng: Locale }) {
  const initialState: RoleOrderState = { message: undefined, status: 'idle' }
  const [_formState, formAction, isPending] = useActionState(orderRoles, initialState)
    ...
    <Section>
      <Container>
        <div className="flex items-center gap-3 py-[2px]">
          <h1 className="!m-0 pb-[2px]"> Roles</h1>
          <Stats total={data?.roles?.length} />
          <IconButton
            aria-label="Create New User"
            render={<LangLink href="/admin/roles/add" lng={lng} />}
          >
        ...
        {items == null || items.length === 0 ? (
          <div className="caption-bottom text-center mt-[6vh]">No roles found</div>
        ) : (
          <DraggableSortable
