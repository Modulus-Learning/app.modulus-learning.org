...
} from '@/modules/admin/session/storage'
...
export const withAdminSession: ProxyLayer = (next) => {
  return async (request, event, context) => {
    const { status, payload: session, expires_at_ms } = await readAdminSession(request)
    ...
    const refreshToken = request.cookies.get(config.cookies.adminRefresh.name)?.value
    ...
    if (refreshToken) {
      try {
        ...
        const response = await fetch(`${config.publicServerUrl}/routes/admin/refresh`, {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          if (status === 'success') {
            ...
            context.adminSession = session
            request.cookies.set(config.cookies.adminSession.name, tokens.access.token)
            request.cookies.set(config.cookies.adminRefresh.name, tokens.refresh.token)
    ...
    // errors.
    const hasSessionCookie = request.cookies.has(config.cookies.adminSession.name)
    const hasRefreshCookie = request.cookies.has(config.cookies.adminRefresh.name)
    if (hasSessionCookie) request.cookies.delete(config.cookies.adminSession.name)
    if (hasRefreshCookie) request.cookies.delete(config.cookies.adminRefresh.name)
    const response = await next(request, event, context)
    if (hasSessionCookie) response.cookies.delete(config.cookies.adminSession.name)
    if (hasRefreshCookie) response.cookies.delete(config.cookies.adminRefresh.name)
    return response
  }
}
