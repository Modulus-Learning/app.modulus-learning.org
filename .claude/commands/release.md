---
name: release
description: Release @modulus-learning/agent — changeset → version-packages → lint → release commit → npm publish + tag → sync develop/main → optional GitHub release.
...
The command is idempotent at every step — re-running after a partial failure
...
If `$ARGUMENTS` is one of `patch` / `minor` / `major`, use that. Otherwise ask via
`AskUserQuestion`:
...
no `-s` (see `.claude/rules/conventional-commits.md`). Then `git push` on the
current branch.
